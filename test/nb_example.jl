...
begin # convert x/y to mne position
    info = pycall(PyMNE.io.meas_info.create_info, PyObject, string.(1:size(pos, 2));
                  sfreq=256, ch_types="eeg") # fake info
...
begin
    ...
    JLD2.save(joinpath(@__DIR__, "example.jld2"),
              Dict("data" => data, "pos" => pos, "pos2" => pos2))
end
