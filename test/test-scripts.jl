...
info = pycall(PyMNE.mne.create_info, PyObject, TopoPlots.CHANNELS_10_20, 120;
              ch_types="eeg")
...
display(PyMNE.viz.plot_topomap(rand(19), layout.pos))
...
info = pycall(PyMNE.io.meas_info.create_info, PyObject, string.(1:size(pos, 2)); sfreq=256,
              ch_types="eeg") # fake info
...
PyMNE.viz.plot_topomap(data[:, 340, 1], get_info(raw); names=string.(1:size(pos, 2)),
                       show_names=true, vmin=-0.7, vmax=0.7)
...
begin
    ...
    data = zeros(length(positions))
    data[1] = 1.0
    fig = eeg_topoplot(data, nothing; positions=positions, label_scatter=(markersize=20,),
...
begin
    ...
    f, ax, p = Makie.scatter(pos_extra; color=data_extra, markersize=10,
                             axis=(aspect=DataAspect(),))
