...
function mne_topoplot(fig, data, positions)
    ...
    f = PyPlot.figure()
    PyMNE.viz.plot_topomap(data, posmat; sphere=1.1, extrapolate="box", cmap="RdBu_r", sensors=false,
                           contours=6)
...
function compare_to_mne(data, positions; kw...)
    f, ax, pl = TopoPlots.eeg_topoplot(data, nothing;
                                       interpolation=ClaughTochter(; fill_value=NaN,
                                                                   tol=0.001,
                                                                   maxiter=1000,
                                                                   rescale=false),
...
begin # empty eeg topoplot
    f, ax, pl = TopoPlots.eeg_topoplot(1:length(TopoPlots.CHANNELS_10_20), TopoPlots.CHANNELS_10_20;
...
begin # df_timebin
    ...
    x = TopoPlots.df_timebin(df, 0.5)
    @test string.(x.time[1]) == "[0.0, 0.5)"
    @test string.(x.time[4]) == "[1.5, 1.9]"

    x = TopoPlots.df_timebin(df, 0.1)
    @test nrow(x) == 20 - 1

    df = DataFrame(:erp => repeat([1], 20 * 3), :time => repeat((0:19) ./ 10, 3),
                   :label => repeat([1, 2, 3], 20))
    x = TopoPlots.df_timebin(df, 0.1; grouping=[:label])
    @test nrow(x) == (20 - 1) * 3
end

begin #eeg_topoplot_series Matrix
    f = Makie.Figure(; resolution=(1000, 1000))
    TopoPlots.eeg_topoplot_series!(f[1, 1], data[:, :, 1], 40; positions=positions)
...
begin #eeg_topoplot_series DataFrame
    ...
    f = Makie.Figure(; resolution=(1000, 1000))
    TopoPlots.eeg_topoplot_series!(f, df, 0.1; positions=positions)
...
begin # eeg_topoplot_series row/col
    ...
    f = Makie.Figure(; resolution=(1000, 1000))
    TopoPlots.eeg_topoplot_series!(f, df, 0.1; positions=positions, col=:time, row=:category)
...
begin
    ...
    Makie.scatter!(ax, positions[1:19]; color=data[1:19, 340, 1], markersize=5, strokecolor=:white,
                   strokewidth=0.5)
    Makie.lines!(ax, rect)
